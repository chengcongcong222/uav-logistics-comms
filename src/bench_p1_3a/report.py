...
PIDS=('A11','AN01','A03')
REVISIONS={'A11':'v2','AN01':'v3','A03':'v2'}
...
def main():
    ...
    for pid in PIDS:
        root=OUT/'runs'/REVISIONS[pid]/pid
        ...
        for r in read(root/'extreme_results.json'):
            ...
            if r['termination_status']==2 or r['candidate_error']:
                anomalies.append(dict(pid=pid,anchor=axis,revision=REVISIONS[pid],status=r['termination_status'],candidate_error=r['candidate_error'],
    ...
    write(OUT/'extreme_results.json',dict(authority=REVISIONS,results=rows,
    ...
    write(OUT/'solver_bounds.json',dict(accepted_policy='Replayable exact-rational LP certificates; integer ceiling for relay count. Raw BnB bounds are reported separately, not used for final claims.',
        accepted=rows,raw_authoritative=raw,prior_run_files=[rel(p) for p in sorted((OUT/'runs').glob('*/*/extreme_results.json'))],
        exact_rational_certificates=certificates,universal_transport_workload_bounds=resources,anomalies=anomalies,
        historical_invalid_bound=read(OUT/'diagnostics/v2_AN01_abort.json'),
        historical_v1_status_warning='AN01 energy reported infeasible despite embedded verified warm; do not accept that status as a proof'))
    all_packages=[]
    for p in sorted((OUT/'runs').glob('*/**/solutions/*/witness.json')):
        vp=p.parent/'validation_0.5.json'
        v=read(vp) if vp.exists() else None
        all_packages.append(dict(package=rel(p.parent),witness_sha256=digest(p),validation_sha256=digest(vp) if v else None,
            status=v['status'] if v else 'MISSING_INDEPENDENT_VALIDATION',metrics=read(p)['metrics']))
    write(OUT/'independent_audit.json',dict(hash_bound_p1_2_inputs=len(inputs['sources']),input_admission=rel(OUT/'inputs.json'),
        ...
        all_revisions_exported_packages=all_packages,all_revisions_exported_count=len(all_packages),
    ...
    snapshots=[]
    for p in (OUT/'source_snapshot_v3').glob('*.py'):
        if p.name not in ('run.py','model.py','admit.py','test_model.py'):continue
    ...
    for pid,revision in REVISIONS.items():
        config=read(OUT/'runs'/revision/pid/'config.json')
        snapshot=OUT/('source_snapshot_'+revision)
        for name in ('run.py','model.py'):
            snapshots.append(dict(path=f'{revision}/{pid}/{name}',matches_executed_snapshot=digest(snapshot/name)==config['source_hashes']['src/bench_p1_3a/'+name]))
    ...
    write(OUT/'environment.json',dict(python=platform.python_version(),scipy=scipy.__version__,numpy=numpy.__version__,solver='SciPy bundled HiGHS',
        ...
        authority=REVISIONS,note='Three revisions retained. Time limits are wall-clock solver limits, process CPU measured separately. This is not an equal-CPU benchmark.',timings=timings))
    ...
    md += ['', 'A11 的新低能耗见证 A11_P13A_003 完工约 11082.025 s，J_late≈9.99×10⁻⁵，满足本轮 10⁻⁴ 硬层但不是数学上的严格零。它以更长完工换取能耗降低，不能称为支配 P1.2 的 7013.083 s 主对照。']
    md += ['', 'A11 能耗方向的原始 HiGHS 返回 status=0，dual bound≈69.874306641 kWh，与该验证见证的数值差约 2.5×10⁻¹² kWh。这个求解器报告保留在原始结果中；考虑 AN01 暴露出的矛盾状态，本轮最终独立证书口径保守采用 LP 下界 69.537006858 kWh、gap≈0.4827%，没有把未经独立复演的分支定界树当作数学证明。']
    md += ['',f"权威运行的新求解器候选中有 {len(new)} 套通过原独立 Q3 验证；包含诊断轮在内共导出 {len(all_packages)} 个执行包，逐包验证状态及哈希均列入 independent_audit.json。另有 54 个 P1.2 内部输入经提交内清单与验证文件哈希绑定，其中 6 个代表包重新执行完整核验（5 个满足零迟到硬层，1 个 A03 仅作为软迟到历史对照）。没有把 54 个哈希核对说成 54 次新物理验证。",
        '', '## 数学域与实现限制','',
