"""Own patterns from local inputs, jointly chosen with typed resource schedules.
...
def solve(pool,pid,horizon=7200,step=180,limit=60,zero=True,single=False,challenge=None):
    ...
    sol=milp(np.array(cost),integrality=np.ones(len(cost)),bounds=Bounds(np.zeros(len(cost)),bound),constraints=LinearConstraint(matrix,lo,hi),options=dict(time_limit=limit,mip_rel_gap=.01,presolve=False))
