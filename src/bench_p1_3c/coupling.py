...
from src.bench_p1_3c.fixed import run
...
def transport_master(e,core,tasks,cuts,path,hint,limit=30,tabus=None,site_relaxation=False):
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('pid');ap.add_argument('--iterations',type=int,default=5);ap.add_argument('--site-relaxation',action='store_true');args=ap.parse_args()
    ...
    stage='C2_SITE' if args.site_relaxation else 'C2'
    ...
    for it in range(args.iterations):
        label=f'{stage}_RELAY_{it:02d}';mr=transport_master(e,core,tasks,cuts,root/f'master_{it:02d}',shifts,limit=60 if args.site_relaxation else 30,tabus=tabus,site_relaxation=args.site_relaxation)
