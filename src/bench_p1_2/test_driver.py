...
from src.bench_p1_2.driver import Anchor, AXES, run, remap_sites
...
class AnchorContract(unittest.TestCase):
    ...
    def test_site_serialization_alias(self):
        from types import SimpleNamespace
        e=SimpleNamespace(sites={'new':dict(x_m=1.,y_m=2.,agl_m=300.)})
        w=dict(groups=[dict(site_id='old')],selected=[dict(site_id='old')])
        out,mapping=remap_sites(e,w,[dict(site_id='old',x_m=1.,y_m=2.+4e-10,agl_m=300.)])
        self.assertEqual(out['groups'][0]['site_id'],'new')
        self.assertEqual(w['groups'][0]['site_id'],'old')
        with self.assertRaises(ValueError):
            remap_sites(e,w,[dict(site_id='old',x_m=1.,y_m=2.1,agl_m=300.)])
