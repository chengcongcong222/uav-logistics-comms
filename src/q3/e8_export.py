"""Write self-contained E8 L1 execution packages; E6 artifacts stay immutable."""
import shutil
...
def export_solution(engine,result,solution_id,destination=None):
    ...
    if dest.resolve().parent==(OUT/'solutions').resolve():
        write_json(OUT/'validation.json',dict(gate='E8_VALIDATION_REQUIRED',changed_solution=solution_id,
                                            reason='Canonical execution rewritten; rerun independent audits'))
    shutil.copyfile(E6/'input_manifest.json',dest/'input_manifest.json')
    write_csv(dest/'guarded_atomic_tasks.csv',engine.atoms.to_dict('records'))
    shutil.copyfile(gamma_dir(engine.gamma)/'candidate_sites.csv',dest/f'candidate_sites_{pid}.csv')
    write_json(dest/'scenario.json',dict(Gamma_C_db=engine.gamma,threshold_applies_to='DIRECT_AND_BOTH_RELAY_HOPS'))
    ...
    write_json(dest/f'plan_status_{pid}.json',dict(status='E8_CANDIDATE_PENDING_INDEPENDENT_VALIDATION'))
    ...
    metrics.update(solution_id=solution_id,level=result['level'],search_objective=result['objective'],
                   J_late_budget=result['J_late_budget'],Gamma_C_db=engine.gamma,global_optimum_proven=False,
