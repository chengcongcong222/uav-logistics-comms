...
def load():
    t=read(SOURCE/'transport_sorties_P01.csv');r=read(SOURCE/'relay_sorties_P01.csv');g=read(SOURCE/'communication_guarantee_P01.csv');b=read(SOURCE/'box_delivery_P01.csv')
    ...
    for x in read(SOURCE/'transport_battery_calendar_P01.csv').itertuples():
...
def main():
    ...
    config=dict(source_solution='Q3E7_001',Gamma_C_db=0,k=[2,3],selected=selected,balanced_alternatives=balanced,
    ...
    write(OUT/'input_manifest.json',dict(source_commit='49b63673ff13b8ff925c5d4e955d06484a7ebf54',hashes={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sources if p.is_file()}))
