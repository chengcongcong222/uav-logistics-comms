#!/usr/bin/env python3
"""Independent E7 L1 validator: no scheduler feasibility flags or routines used."""
...
def validate_plan(pid,step=0.5):
    ...
    for r in rows.itertuples():
        ...
        rg=checker.backhaul(s);require(rg>=0,'Relay backhaul outage');backhaul_values[r.site_id]=rg
        for aid in ids:
    ...
    result=dict(pareto_id=pid,status='E7_PLAN_INDEPENDENTLY_VALIDATED',step_s=step,hard_violations=0,
...
if __name__=='__main__':
    ...
    try:
        ...
        target.write_text(json.dumps(dict(status='E7_PLAN_VALIDATION_FAILED',error=str(exc)),indent=2)+'\n');raise
