#!/usr/bin/env python3
"""Independent E8 L1 validator: no scheduler feasibility flags or routines used."""
...
def validate_transport(pid,shifts):
    ...
    for sid,row in flights.iterrows():
        ...
        require(row.preparation_start_s>=-1e-7,'Preparation before time zero')
        for field in ['uav_type','service_sequence']:require(row[field]==old[field],f'Changed transport structure {sid}/{field}')
        require(uavs.loc[row.uav_id].uav_type==row.uav_type,'UAV type mismatch')
        require(row.battery_id.startswith(f'BAT-{row.uav_type}-'),'Battery type mismatch')
    ...
    uav_calendar=read(OUT/f'transport_uav_calendar_{pid}.csv').set_index('sortie_id')
    require(uav_calendar.index.is_unique and set(uav_calendar.index)==set(flights.index),'UAV calendar coverage')
    for sid,row in flights.iterrows():
        require(uav_calendar.loc[sid].uav_id==row.uav_id,'UAV calendar ID')
        close(uav_calendar.loc[sid].busy_start_s,row.preparation_start_s,'UAV calendar start')
        close(uav_calendar.loc[sid].busy_end_s,row.return_s,'UAV calendar return')
    ...
    for row in records:
        require(declared.loc[row['sortie_id']].battery_id==row['battery_id'],'Battery calendar ID')
...
def validate_plan(pid,step=0.5):
    tick=time.monotonic()
    gamma=float(json.loads((OUT/'scenario.json').read_text())['Gamma_C_db'])
    require(gamma in [0.,2.,4.,6.],'Unconfigured robustness budget')
    checker=IndependentAudit()
    # Rebase only the independent interval comparator, then restore physical
    # budgets. This makes its observed sign-boundary refinement target Gamma.
    physical_interval=checker.interval
    def robust_interval(*args,**kwargs):
        original=checker.budgets['transport<->relay']
        try:
            checker.budgets['transport<->relay']=original-gamma
            return physical_interval(*args,**kwargs)+gamma
        finally:checker.budgets['transport<->relay']=original
    checker.interval=robust_interval
    ...
    require(atoms.index.is_unique,'Duplicate robust task identity')
    for aid,t in atoms.iterrows():
        require(t.transport_sortie_id in flights.index,'Unknown robust task flight')
        flight=flights.loc[t.transport_sortie_id];delta=shifts[t.transport_sortie_id]
        require(flight.takeoff_s-delta-1e-6<=t.service_start_s<t.service_end_s<=flight.return_s-delta+1e-6,'Task outside original flight')
    ...
    for sid,part in guarantee.groupby('transport_sortie_id'):
        prior=-float('inf')
        for row in part.sort_values('service_start_s').itertuples():
            require(row.service_start_s>=prior-1e-7,'Overlapping robust assignment interiors')
            prior=row.service_end_s
    ...
    relay_energy=0.;min_margin=float('inf');min_mc=float('inf');seen=[];backhaul_values={}
    ...
    for r in rows.itertuples():
        ...
        rg=checker.backhaul(s);require(rg>=gamma,'Relay backhaul below Gamma');backhaul_values[r.site_id]=rg
        for aid in ids:
            ...
            require(margin>=gamma,f'Relay access below Gamma {aid}')
    ...
    bad=[];full_samples=0;relayed_samples=0;direct_samples=0;full_refinements=0;direct_margins=[];twohop_margins=[]
    for sid,flight in flights.iterrows():
        ...
        for a,b in zip(coarse[:-1],coarse[1:]):
            ...
            if va[1]!=vb[1] or (va[0]>=gamma)!=(vb[0]>=gamma):pending.append((a,b))
        while pending:
            a,b=pending.pop()
            if b-a<=.1:continue
            ...
            for x,y in [(a,mid),(mid,b)]:
                ...
                if vx[1]!=vy[1] or (vx[0]>=gamma)!=(vy[0]>=gamma):pending.append((x,y))
        for t in sorted(times):
            direct,_,xyz=direct_at(t)
            full_samples+=1
            if direct>=gamma:
                direct_samples+=1;direct_margins.append(direct);continue
            # Adjacent atomic intervals use the incoming provider at a shared
            # endpoint; both link intervals are independently checked including
            # that endpoint. No positive-duration overlapping service is allowed.
            ...
            if value<gamma:bad.append(dict(sortie_id=sid,time_s=float(t),reason='RELAY_ACCESS_BELOW_GAMMA',access_margin_db=value))
            else:
                relayed_samples+=1;twohop_margins.append(min(value,backhaul_values[assignment.site_id]))
    ...
    close(metrics['Gamma_C_db'],gamma,'Declared robustness budget')
    ...
    result=dict(Gamma_C_db=gamma,minimum_twohop_robust_slack_db=min_mc-gamma,pareto_id=pid,status='E8_PLAN_INDEPENDENTLY_VALIDATED',step_s=step,hard_violations=0,
    ...
    result['margin_distribution_db']={name:{str(q):float(np.quantile(values,q)) for q in [0,.01,.05,.25,.5,.75,.95,1]} for name,values in [('direct_active_path',direct_margins),('relayed_active_path',twohop_margins)] if values}
    if step==.5:
        metrics['communication_uncovered_duration_s']=0.;metrics['communication_validation']=result['status'];metrics['conclusion']=result['status'];metrics['full_flight_margin_distribution_db']=result['margin_distribution_db']
    ...
    artifacts += [OUT/'scenario.json',OUT/'guarded_atomic_tasks.csv',OUT/f'joint_metrics_{pid}.json']
    result['artifact_sha256']={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in artifacts}
    return result


if __name__=='__main__':
    parser=argparse.ArgumentParser();parser.add_argument('--directory',required=True);parser.add_argument('--plan',default='P01');parser.add_argument('--step',type=float,default=.5)
    args=parser.parse_args();OUT=Path(args.directory).resolve();target=OUT/f'validation_{args.plan}_{args.step}.json'
    try:
        result=validate_plan(args.plan,args.step)
        target.write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result,indent=2),flush=True)
    except Exception as exc:
        target.write_text(json.dumps(dict(status='E8_PLAN_VALIDATION_FAILED',error=str(exc)),indent=2)+'\n');raise
