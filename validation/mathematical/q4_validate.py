...
import pandas as pd
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'results/q4';SOURCE=ROOT/'results/q3/e7/solutions/Q3E7_001';DATA=ROOT/'data/processed'
...
def validate():
    config=json.loads((OUT/'config.json').read_text());require(config['Gamma_C_db']==0 and config['source_solution']=='Q3E7_001','Wrong Q4 authority')
    manifest=json.loads((OUT/'input_manifest.json').read_text())
    for f,h in manifest['hashes'].items():require(hashlib.sha256((ROOT/f).read_bytes()).hexdigest()==h,'Changed source '+f)
    sourceaudit=json.loads((SOURCE/'validation_P01_0.5.json').read_text());require(sourceaudit['status']=='E7_PLAN_INDEPENDENTLY_VALIDATED','Source Q3 not validated')
    for f,h in sourceaudit['artifact_sha256'].items():require(hashlib.sha256((ROOT/f).read_bytes()).hexdigest()==h,'Stale source audit '+f)
    t=read(SOURCE/'transport_sorties_P01.csv').set_index('sortie_id');r=read(SOURCE/'relay_sorties_P01.csv').set_index('relay_sortie_id');g=read(SOURCE/'communication_guarantee_P01.csv');b=read(SOURCE/'box_delivery_P01.csv')
    battery=read(SOURCE/'transport_battery_calendar_P01.csv').set_index('sortie_id');stock={}
    ...
    result=dict(gate='Q4_INDEPENDENT_PARTITIONS_READY',source_solution='Q3E7_001',Gamma_C_db=0,unsplittable_blocks=len(components),enumerated_partitions=counts,
        exact_pareto_partitions=front_counts,validated_packages=packages,all_partition_resource_minima_certified=True,
        scope='EXACT_FOR_FIXED_E7_001_TASKS_AND_WHOLE_RELAY_DEPENDENCY_PARTITIONS',
    ...
    files += [ROOT/'src/q4/partition.py',ROOT/'validation/mathematical/test_q4_regressions.py',Path(__file__)]
    for optional in ['src/q4/report.py','docs/model/Q4_PARTITION_SEMANTICS.md','results/q4/Q4_REPORT.md','results/q4/Q4_GPT_SYNC.md']:
        p=ROOT/optional
