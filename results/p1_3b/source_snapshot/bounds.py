...
def build(p,t):
    ...
    for i,r in enumerate(patterns):
        ...
        x=m.var(f'pattern_count_{i}',0,ub,integer=True);s=m.var(f'sum_preparation_starts_{i}',0,up(F(max(0,cap))*ub) if ub else 0)
        ...
        for c,n in r['counts']:
            w=classes[c]['key'][5]*n;ell=m.var(f'aggregate_late_{i}_{c}',0,up(F(1e-4)/F(w)));lateness.append((ell,w))
            m.row({s:1,x:down(F(r['delivery'][str(c)])-F(classes[c]['key'][4])),ell:-1},hi=0,label='Jensen_lower_bound_on_total_lateness')
