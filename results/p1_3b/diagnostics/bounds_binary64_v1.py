...
def build(p,t):
    ...
    horizon=max(max(0,latest(r,classes))+r['duration'] for r in patterns)+1.
    ...
    for i,r in enumerate(patterns):
        cap=latest(r,classes);ub=multiplicity(r,classes) if cap>=0 else 0
        ...
        objectives['J_norm'][x]=sum(n*classes[c]['key'][5]*r['delivery'][str(c)]/classes[c]['key'][4] for c,n in r['counts'])/den
        objectives['J_norm'][s]=sum(n*classes[c]['key'][5]/classes[c]['key'][4] for c,n in r['counts'])/den
        for c,n in r['counts']:
    ...
    for typ in 'ABC':
        ...
        m.row({**{xs[i]:patterns[i]['duration']+patterns[i]['charge']-patterns[i]['prep'] for i in ids},T:-b},hi=b*charge,label='BATTERY_WORKLOAD_WITH_TAIL')
        ...
        for q in range(1,33):
            ...
            row={starts[i]:-patterns[i]['duration']/scale for i in ids}
            row.update({xs[i]:(w0/n*patterns[i]['duration']-.5*patterns[i]['duration']**2)/scale for i in ids})
            m.row(row,hi=w0*w0/(2*n*scale),label='PARALLEL_MACHINE_WORKLOAD_SQUARE_TANGENT')
        ...
        for h in deadlines:
            for resource,count in [('uav',n),('battery',b)]:
                ...
                for i in ids:
                    ...
                    forced=max(0,min(length,h-latest(r,classes)-offset));row[xs[i]]=forced
