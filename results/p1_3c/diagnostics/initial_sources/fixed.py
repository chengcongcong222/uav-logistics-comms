...
def solve_master(gen,cols,path,phase='count',limit=30):
    ...
    sol=milp(c,integrality=np.ones(len(c)),bounds=Bounds(0,1),constraints=LinearConstraint(a,lo,hi),options=dict(threads=1,random_seed=SEED,time_limit=limit,mip_rel_gap=0))
    record.update(status=int(sol.status),message=str(sol.message),incumbent=clean(sol.fun),dual_bound=clean(getattr(sol,'mip_dual_bound',None)),gap=clean(getattr(sol,'mip_gap',None)),
        cpu_s=time.process_time()-start,wall_s=time.monotonic()-wall,nodes=clean(getattr(sol,'mip_node_count',None)),conflicts=None)
    chosen=[]
    if sol.x is not None:
        np.save(path/'incumbent.npy',sol.x);chosen=[r for j,r in enumerate(cols) if sol.x[j]>.5]
        record['uncovered_tasks']=[gen.ids[i] for i in range(n) if art and sol.x[len(cols)+i]>.5]
...
def run(e,shifts,label,rounds=3,limit=25,fixed=False):
    ...
    for k in range(rounds+1):
        ...
        cols=list(gen.columns.values());rec,selected,weights=solve_master(gen,cols,root/f'round_{k:02d}',phase='phase1',limit=limit)
