...
def transport_master(e,core,tasks,cuts,path,hint,limit=30,tabus=None):
...
def main():
    ap=argparse.ArgumentParser();ap.add_argument('pid');ap.add_argument('--iterations',type=int,default=5);args=ap.parse_args()
    ...
    root=e.root/'C2';root.mkdir(exist_ok=False);write(root/'task_domain.json',tasks)
    shifts={s:0. for s in core.ids};cuts=conflicts(core,tasks,shifts);write(root/'fixed_time_conflicts.json',cuts)
    log=[];tabus=[];witnesses=[]
    for it in range(args.iterations):
        label=f'C2_RELAY_{it:02d}';mr=transport_master(e,core,tasks,cuts,root/f'master_{it:02d}',shifts,limit=30,tabus=tabus)
        ...
        if new:cuts+=new;row['action']='VALID_OCCUPATION_CUTS_ONLY'
            ...
            result,gen=run(e,shifts,label,rounds=2,limit=20)
