...
def main():
    p=argparse.ArgumentParser();p.add_argument('scope',choices=METHODS);p.add_argument('method');p.add_argument('seed',type=int);p.add_argument('--budget',type=float,required=True);p.add_argument('--core',type=int,required=True);p.add_argument('--root',required=True);p.add_argument('--normalization');a=p.parse_args()
    ...
    reads=set();logging_open=False
    def hook(event,args):
        if event in ['subprocess.Popen','os.system','os.fork','os.posix_spawn']:raise RuntimeError('Unbilled child processes prohibited')
        nonlocal logging_open
        if event=='open' and isinstance(args[0],(str,bytes)) and not logging_open:
            path=os.fsdecode(args[0])
            if path not in reads:
                reads.add(path);logging_open=True
                try:append(root/'input_opens.jsonl',dict(path=path,cpu_s=time.process_time(),mode=args[1],flags=args[2]))
                finally:logging_open=False
    ...
    try:
        ...
        calibration=json.loads(Path(a.normalization).read_text())[a.scope] if a.normalization else None
        pref=Preference(a.scope,root,scales=calibration['scales'] if calibration else None,offsets=calibration['offsets'] if calibration else None)
        ...
        if a.scope=='Q2' and a.method in ['UNIFORM_LNS','ALNS']:
            stage('neighborhood_search')
            if a.method=='UNIFORM_LNS':
                from src.bench_p1.uniform import search as uniform_search
                uniform_search(base,rng,ev,dec,pref,remaining,save)
            else:search(base,rng,ev,dec,pref,remaining,save,adaptive=True)
            return
