...
def run(method,root,pool,base,tab,pref,patterns,remaining,milp,linprog,records,stage):
    ...
    def trkey(r):
        m=r['metrics'];return pref.key(dict(J_late=m['J_late'],J_norm=m['J_norm'],joint_makespan_s=m['makespan'],total_energy_kwh=m['energy'],transport_sorties=m['sorties'],relay_sorties=0),'transport_seed_selection')
    ...
    def save(answer,origin):
        ...
        ident=f'Q3{len(answers)+1:04d}';export_local(e,answer,ident);dest=e.dest/'solutions'/ident
...
def export_local(e,answer,ident):
    ...
    write(dest/f'selected_sorties_{pid}.json',selected);write(dest/f'shifts_{pid}.json',answer['shifts'])
    write(dest/f'plan_status_{pid}.json',dict(status='XB1_Q3_PENDING_INDEPENDENT_VALIDATION'))
    ...
    metrics=json.loads((dest/f'joint_metrics_{pid}.json').read_text());metrics.update(answer['metrics'],Gamma_C_db=0.,solution_id=ident,source_provenance='FRESH_P1_TRANSPORT',global_optimum_proven=False)
    write(dest/f'joint_metrics_{pid}.json',metrics);write(dest/'witness.json',answer)
    write(dest/'scenario.json',dict(Gamma_C_db=0,transport_baseline=pid))
    files=[p for p in e.base.glob('*') if p.suffix in ['.csv','.json'] and not p.name.startswith('validation')]+list((ROOT/'data/processed').glob('*.csv'))+list((ROOT/'data/processed').glob('*.json'))
    write(dest/'input_manifest.json',dict(hashes={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in files}))
