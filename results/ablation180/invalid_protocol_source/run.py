...
import argparse,hashlib,json,math,os,platform,resource,signal,subprocess,sys,time
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--budget',type=float,default=180);p.add_argument('--seeds',nargs='+',type=int,default=[250901,250902,250903,250904,250905]);p.add_argument('--root',default='results/ablation180');a=p.parse_args()
    ...
    config=dict(version='CPU180_FIVE_SEEDS_Q2_V1',scope='Q2_ONLY',methods=methods,seeds=a.seeds,cpu_budget_s=a.budget,cores=cores,parallel_workers=3,core_assignment='rotate by seed index',checkpoint_cpu_cutoff_s=a.budget,supervisor_poll_s=.01,maximum_observed_cpu_overshoot_tolerance_s=.2,secondary_wall_watchdog_s=900,thread_limit=1,
