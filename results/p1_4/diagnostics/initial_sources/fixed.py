...
def matrix(gen,cols,artificial):
    ...
    row=len(lower);lower.append(-np.inf);upper.append(MAX_RELAY)
    for j in range(len(cols)):rr.append(row);cc.append(j);vv.append(1.)
...
def solve_master(gen,cols,path,phase='count',limit=30):
    ...
    objective_scale=1e6 if phase=='energy' else 1
    c=np.r_[np.zeros(len(cols)),np.ones(len(gen.ids))] if art else (np.array([round(r['total_energy_kwh']*objective_scale) for r in cols],dtype=float) if phase=='energy' else np.ones(len(cols)))
    ...
    model.Minimize(sum(int(v)*z[j] for j,v in enumerate(c) if v))
    if art:
    ...
    record.update(status=int(status),message=solver.StatusName(status),incumbent=obj,dual_bound=bound,gap=(obj-bound)/max(1,abs(obj)) if feasible else None,objective_scale=objective_scale,relay_count_cap=MAX_RELAY,joint_time_cap_s=CAP,
...
def run(e,shifts,label,rounds=3,limit=25,fixed=False,guidance=None):
    ...
    for k in range(rounds+1):
        ...
        if rec['incumbent'] is not None and rec['incumbent']<.5:
            ...
            log.append(count)
            energy,lower_energy,_=solve_master(gen,cols,root/'energy_master',phase='energy',limit=limit)
            if lower_energy:chosen=lower_energy
            log.append(energy);break
