...
import numpy as np
import pandas as pd
ROOT=Path(__file__).resolve().parents[2];OUT=ROOT/'results/p1_5a/q4';SOURCE=ROOT/'results/p1_5a/execution/q3/F13/solutions/F13_C2_SITE_B6500_RELAY_00_Q3';DATA=ROOT/'results/reset/geometry'
...
def load():
    t=read(SOURCE/'transport_sorties_F13.csv');r=read(SOURCE/'relay_sorties_F13.csv');g=read(SOURCE/'communication_guarantee_F13.csv');b=read(SOURCE/'box_delivery_F13.csv')
    ...
    for x in read(SOURCE/'transport_battery_calendar_F13.csv').itertuples():
...
def main():
    from src.p1_5a.common import require_admitted
    require_admitted()
    ...
    for k in [2,3]:
        ...
        for i,labels in enumerate(partitions(len(blocks),k),1):
            result=evaluate(labels,data);result['partition_id']=f'F13_Q4_K{k}_{i:03d}';result['redundancy']={kind:result['resources'][kind]-baseline[kind] for kind in KINDS};rows.append(result)
    ...
    config=dict(source_solution='F13_C2_SITE_B6500_RELAY_00_Q3',Gamma_C_db=0,k=[2,3],selected=selected,balanced_alternatives=balanced,
        ...
        fixed_F13_task_domain_only=True,resource_shortages_allowed_as_reported_Q4_outputs=True)
    ...
    write(OUT/'input_manifest.json',dict(source_commit='1af5aeebf6f30bc447bc8e45ee77fead38317c3e',hashes={str(p.relative_to(ROOT)):hashlib.sha256(p.read_bytes()).hexdigest() for p in sources if p.is_file()}))

if __name__=='__main__':
    from src.p1_5a.common import install_guard
    install_guard();main()
