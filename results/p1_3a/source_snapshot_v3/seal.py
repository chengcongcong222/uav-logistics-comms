...
def main():
    ...
    assert len(certs['records'])==24 and certs['all_passed']
    for r in rows:
    ...
    for r in audit['new_solver_candidates']:
        if r['status']=='INDEPENDENTLY_VALIDATED':
            assert r['result']['status']=='P1_Q3_INDEPENDENTLY_VALIDATED'
            assert r['result']['hard_violations']==0
            assert r['result']['uncovered_sample_count']==0
    write(OUT/'integration_check.json',dict(status='PASSED',anchor_records=12,certificate_replays=24,
