...
def main():
    ...
    root=OUT/'admission'
    if root.exists() and not args.resume:raise RuntimeError('Refuse overwrite without explicit resume')
    for pid in PIDS:
        ...
        if target.exists():
            for p in src.iterdir():
                if p.is_file():assert digest(p)==digest(target/p.name)
    ...
    for row in sources:
        ...
        if not existing:shutil.copytree(ROOT/row['package'],dest)
        try:
            if existing:
                result=json.loads((dest/'validation_0.5.json').read_text())
                assert result['status']=='P1_Q3_INDEPENDENTLY_VALIDATED'
                for f,h in result['artifact_sha256'].items():assert digest(ROOT/f)==h
                assert digest(dest/'witness.json')==row['witness_sha256']
            else:result=audit(root,row['pid'],dest)
            row['admission']='FRESH_FULL_Q3_REVALIDATION'
            records.append(dict(id=row['id'],pid=row['pid'],status='VALIDATED',
                      zero_late_eligible=bool(result['J_late']<=1e-4),resumed_completed_audit=existing,result=result))
