...
def main():
    ...
    parser.add_argument('--limit',type=float,default=90);parser.add_argument('--revision',default='v1');args=parser.parse_args()
    ...
    config=dict(mode='STRUCTURE_FIXED_EXACT_AUDIT',pid=args.pid,solver_seed=SEED,formal_seed=False,
                ...
                no_external_targets=True,source_hashes={str(p.relative_to(ROOT)):digest(p) for p in (ROOT/'src/bench_p1_3a').glob('*.py')})
    ...
    for axis in AXES:
        ...
        cutoff=None
        if incumbent:
            cutoff=incumbent['metrics'][axis]+(1e-8 if axis=='J_norm' else 1e-6)
            aa=vstack([a,csr_matrix(c)]).tocsr();ll=np.r_[lo,-np.inf];hh=np.r_[hi,cutoff]
        ...
        sol=milp(c,integrality=np.array(model.integer),bounds=Bounds(model.lb,model.ub),constraints=LinearConstraint(aa,ll,hh),
            options=dict(threads=1,time_limit=args.limit,mip_rel_gap=1e-6,random_seed=SEED,mip_feasibility_tolerance=1e-8))
        ...
        if sol.x is not None:
            ...
            try:
                answer=model.materialize(sol.x,axis)
                if answer['metrics']['J_late']>1e-4:raise ValueError('Numerical candidate outside zero-lateness hard layer')
                residual={k:float(model.objective(k)@sol.x)-answer['metrics'][k] for k in AXES}
                if max(abs(v) for v in residual.values())>2e-5:raise ValueError(('Physical/linear objective mismatch',residual))
        ...
        valid_bounds=[x for x in [solver_lb,cert_lb] if x is not None]
        ...
        row=dict(pid=args.pid,anchor=axis,mode='STRUCTURE_FIXED_EXACT_AUDIT',incumbent=None if incumbent is None else {k:v for k,v in incumbent.items() if k!='witness'},
            ...
            warm_start_mechanism='VERIFIED_EXTERNAL_INCUMBENT_PLUS_OBJECTIVE_CUTOFF; scipy.milp has no supplied primal MIP-start in this driver',
