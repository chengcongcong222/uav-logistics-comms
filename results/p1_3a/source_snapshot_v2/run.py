...
def main():
    ...
    parser.add_argument('--limit',type=float,default=90);parser.add_argument('--revision',default='v2');args=parser.parse_args()
    ...
    for axis in AXES:
        ...
        valid_bounds=[x for x in [None if contradiction else solver_lb,cert_lb] if x is not None]
        ...
        row=dict(pid=args.pid,anchor=axis,mode='STRUCTURE_FIXED_EXACT_AUDIT',incumbent=None if incumbent is None else {k:v for k,v in incumbent.items() if k!='witness'},
            ...
            solver_infeasibility_conflicts_with_verified_warm=contradiction,
